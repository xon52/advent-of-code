...
const run = (puzzle) => {
	...
	let registerA = data[0].split(': ')[1];
	let registerB = data[1].split(': ')[1];
	let registerC = data[2].split(': ')[1];
	const instructions = data[4].split(': ')[1].split(',');
	let output = [];

	const result = process(0, instructions, registerA, registerB, registerC, output);
