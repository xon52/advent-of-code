...
if (isMainThread) {
	...
	// Work backwards on last column
	let result = 0;
	for (let c = rows.length - 1; c > 0; c--) {
		rows[c - 1].push(rows[c - 1][rows[c - 1].length - 1] + result);
		result = rows[c - 1][rows[c - 1].length - 1];
	}
	parentPort.postMessage(rows[0][rows[0].length - 1]);
}
